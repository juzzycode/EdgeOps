...
export const serverConfig = {
  ...
  historyPollingIntervalMs: Number(process.env.EDGEOPS_HISTORY_POLLING_INTERVAL_MS ?? 15 * 60 * 1000),
  email: {
    mode: String(process.env.SMTP_MODE ?? process.env.EDGEOPS_EMAIL_MODE ?? 'smtp').trim().toLowerCase(),
    host: process.env.EDGEOPS_SMTP_HOST ?? '',
    port: Number(process.env.EDGEOPS_SMTP_PORT ?? 587),
    secure: process.env.EDGEOPS_SMTP_SECURE === 'true',
    user: process.env.EDGEOPS_SMTP_USER ?? '',
    password: process.env.EDGEOPS_SMTP_PASSWORD ?? '',
    from: process.env.EDGEOPS_SMTP_FROM ?? process.env.EDGEOPS_SMTP_USER ?? '',
    mailgun: {
      apiKey: process.env.MAILGUN_API ?? process.env.EDGEOPS_MAILGUN_API_KEY ?? '',
      domain: process.env.MAILGUN_DOMAIN ?? process.env.EDGEOPS_MAILGUN_DOMAIN ?? '',
      from: process.env.MAILGUN_FROM ?? process.env.EDGEOPS_MAILGUN_FROM ?? process.env.EDGEOPS_SMTP_FROM ?? process.env.EDGEOPS_SMTP_USER ?? '',
      apiBaseUrl: (process.env.MAILGUN_API_BASE_URL ?? process.env.EDGEOPS_MAILGUN_API_BASE_URL ?? 'https://api.mailgun.net').replace(/\/$/, ''),
    },
  },
